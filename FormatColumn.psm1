...
#>
    ...
    if ($null -ne $GroupBy)
    {
        ...
        $expr = $formatStr = $null
        if (Get-Variable -Name inputData_*) { Remove-Variable -Name inputData }
    }
    $inputDataVbs = (Get-Variable -Name inputData*)
    ...
    if (-not $ColumnCount)
    {
        $ColumnCount = ($inputDataVbs | ForEach-Object {
            ...
            if ($_.Value.Count -lt $colCount) { $colCount = $_.Value.Count }
            if ($MaxColumnCount -and $MaxColumnCount -lt $colCount) { $colCount = $MaxColumnCount }
            $colCount
    ...
    foreach ($inputDataVb in $inputDataVbs)
    {
        ...
        if (-not $inputData) # If GroupBy parameter, add grouping label
        {
            Write-Output "   ${groupLabel}: $($inputDataVb.Name -replace '^inputData_')"
            if ($PSEdition -eq 'Desktop') { Write-Output "`n" }
            else                          { Write-Output "" }
        }
