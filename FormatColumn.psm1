...
.EXAMPLE
 ...
 Get-Process | Format-Column -Property @{exp='ProcessName'}     # hashtable string expression
 Get-Process | Format-Column -Property @{exp={$_.ProcessName}}  # hashtable scriptblock expression
#>
    ...
    if ($Property) {
        if ($Property -is [Hashtable]) {
            ...
            if ($expression) {
                ...
                elseif ($expression -is [ScriptBlock]) {
                    ...
                    trap { Write-Error "Error when processing Property." -Category 5 -EA 1 }
                }
        ...
        elseif ($Property -is [String]) { $inputData = $inputData | ForEach-Object { $_.$Property } }
        
        if ($inputData) { $inputData = $inputData | ForEach-Object { $_.ToString() } }
        else { return }
    }
    else { $inputData = $inputData | ForEach-Object { $_.ToString() } }
